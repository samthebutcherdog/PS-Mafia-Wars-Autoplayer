...
// Dom related Variable
var strFrameID;									// Name of current frame;
var iFrameCanvas;								// iFrameCanvar for Mafia Wars
...
/****************************** Code that will only run in Facebook  ***************************/
...
function FaceBook() {
	...
	// get the iframe information for mafia wars and make it the top window
	iFrameCanvas  = xpathFirst('//iframe[@name="mafiawars"]');
  if (iFrameCanvas) {
    setFBParams();
    window.location.replace(iFrameCanvas.src);
  } else {
	  GM_log('no iframe found');
  }
}

/****************************** Code that will only in popups  ***************************/
...
function MWAP_Main() {
	...
	// mini icons
	var	searchIcon, lootbagIcon, playIcon, infoIcon, attackIcon, closeButtonIcon, updateGoodIcon, pausedMessageImage, 
		experienceIcon, omgIcon, pauseIcon, defenseIcon, staminaIcon, huhIcon, updateBadIcon, processIcon, energyPackIcon, 
		energyIcon, killedMobsterIcon, yeahIcon, mafiaHatIcon, plussignIcon, cashIcon, cashBangkokIcon, cashCubaIcon,
		cashMoscowIcon, cashVegasIcon, healthIcon, foundIcon, warningIcon, chickenIcon, hideIcon, bgTabImage,
	...
	function saveSettings() {
	  ...
	  // Place ONLY checkbox and radiobox elements here to save them:
	  if(!isGMChecked('TestChanges')){
	    saveCheckBoxElementArray(['autoClick','autoLog','logPlayerUpdates','hideAttacks','attackCritical',
	                            'autoMission','allowEnergyToLevelUp','autoHeal','forceHealOpt3','forceHealOpt4','forceHealOpt5',
	                            ...
	                            'autoWarBetray','hideGifts','autoSecretStash','autoIcePublish','burstJob','autoLevelPublish',
	                            'autoAchievementPublish','autoShareWishlist','autoGiftAccept','autoDailyChecklist','hideActionBox','showPulse',
	...
	function createLogBox() {
	  ...
	  makeElement('div', statPassiveElt, {'id':'passivetotalExp', 'style':'position: absolute; right: 329px; bottom: 50px; font-size: 10px; font-weight: 600;color: #52E259;'}).appendChild(document.createTextNode(makeCommaValue(GM_getValue('passivetotalFightExpInt', 0) + GM_getValue('passivetotalJobExpInt', 0))));
	  ...
	  makeElement('div', statPassiveElt, {'id':'passivetotalFightExp', 'style':'position: absolute; right: 240px; bottom: 50px; font-size: 10px; font-weight: 600;color: #52E259;'}).appendChild(document.createTextNode(makeCommaValue(GM_getValue('passivetotalFightExpInt', 0))));
	  ...
	  makeElement('div', statPassiveElt, {'id':'passivetotalJobExp', 'style':'position: absolute; right: 141px; bottom: 50px; font-size: 10px; font-weight: 600;color: #52E259;'}).appendChild(document.createTextNode(makeCommaValue(GM_getValue('passivetotalJobExpInt', 0))));
	...
	function createStaminaTab() {
	 ...
	 // Fight till health is gone then Rob, was in fighting section, but needs to be visible ALL the time to avoid problems
	  makeElement('br', rhs, {'class':'hide'});
	  ...
	  label = makeElement('label', rhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode('Burn '));
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'style':'width: 2em; border: #781351;', 'value':GM_getValue(id, '3')});
	  label = makeElement('label', rhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode(' points '));
	  ...
	  title = 'Reattack if this amount is gained';
	  id = 'reattackThreshold';
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'maxlength':6, 'style':'width: 45px; border: #781351;', 'value':GM_getValue(id, '65000'), 'size':'1'});
	  label = makeElement('label', rhs, {'for':id, 'title':title,'style':'margin-left: 0.5em;'});
	  ...
	  label = makeElement('label', lhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode('Maximum level:'));
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'maxlength':5, 'style':'width: 30px; border: #781351;', 'value':GM_getValue('fightLevelMax', '100'), 'size':'1'});
	  ...
	  label = makeElement('label', lhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode('Maximum mafia:'));
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'style':'width: 30px; border: #781351;', 'value':GM_getValue('fightMafiaMax', '501'), 'size':'1'});
	  ...
	  label = makeElement('label', lhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode('Minimum mafia:'));
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'style':'width: 30px; border: #781351;', 'value':GM_getValue('fightMafiaMin', '1'), 'size':'1'});
	  ...
	  title = 'Avoid opponents known to be Top Mafia bodyguards. This may ' +
	          'decrease the frequency of losses due to critical hits.';
	  id = 'fightAvoidBodyguards';
	  makeElement('input', rhs, {'type':'checkbox', 'id':id, 'title':title, 'style':'vertical-align:middle', 'value':'checked'}, 'fightAvoidBodyguards');
	  label = makeElement('label', rhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode(' Avoid Top Mafia bodyguards'));
	  ...
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'maxlength':6, 'style':'width: 45px; border: #781351;', 'value':GM_getValue('reattackThreshold', '65000'), 'size':'1'});
	  label = makeElement('label', rhs, {'for':id, 'title':title,'style':'margin-left: 0.5em;'});
	  label.appendChild(document.createTextNode('bucks'));
	  ...
	  label = makeElement('label', lhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode('Minimum bounty:'));
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'style':'width: 7em; border: #781351;', 'value':GM_getValue('hitmanBountyMin', '0')});
	  ...
	  label = makeElement('label', lhs, {'for':id, 'title':title});
	  label.appendChild(document.createTextNode('Bounty:'));
	  makeElement('input', rhs, {'type':'text', 'id':id, 'title':title, 'style':'width: 7em; border: #781351;', 'value':GM_getValue('autoHitListBounty', '0')});
	...
	*/
	    ...
	    switch (action) {
	      ...
	      case 'deposit':
	        ...
	        // Unknown JSON response
	          ...
	          quickBankFail = true;
	          addToLog('warning Icon', 'Unknown JSON response:<br>' + respTxt);
	        }
	        break;
	...
	function load_uris_icons() {
		//load data uris for mini icon usage
		searchIcon 				= '<img src="' +
		...
		cashIcon					=	'<img src="data:image/gif;base64,' +
												'R0lGODlhEAAQAKU/AHS3k3W0OxIbEtW8BTN7S9XquYnDlVKSNFObXEpzVmelcPXkAXCNXpzUpWyoRPbbAChtEcfFJqm9c7jaimOXe5iiGWyqiVaHboe3Y4i9k0h4YGWTaK3Wd0V5EydCJZzKbDxkIluPc+Lxz6nU' +
												'b2JbD+zMA5XCXJG9cj+MW3rAmnCxhVB/Zk+bckBTSN7bAsrDBMa/E9XDD1NcLkt2J6u5PMrlqGOOaIS7V6TObKjIf22NC4K6TmOHKJ6rQfLPAP///yH5BAEAAD8ALAAAAAAQABAAAAaMwJ9wSCwaj8hkUXBgMARK' +
												'UKAm+mB4HZnxEJiIODkJBLKggTxDAgKzM/U6EFdE8rmdZp7VikCoDHQvc14FIwcEIYgrMAsPFQ44BQUcDgo2CRQUFzAPCwkoCA4mAQgNCT8CHhoxPg8bFhYsCAoNBlBDJAMlCgC8AAYZaEYCLRQAKSkZG0qnCQAq' +
												'tsvRSUEAOw==' +
												'" />';
		
		cashBangkokIcon		=	'<img src="' +
												'data:image/gif;base64,' +
		...
		cashCubaIcon			=	'<img src="' +
												'data:image/gif;base64,' +
												'R0lGODlhEAALANU/AMa6tcCbk7lpirKkmxRvU+Tc1ChYKlclM8zDuiu1J+LSzFmYYxX1AHQzUnBsbB0XF9PGu8Jcje3i3t/X0NrMxLt1puvh2Av1AfHP4eq508psmPPm49qKtdZ3qY1ebu3j2bQ7cBjVBWaHZ9Rv' +
												'pbNce7tTf9e/uuDOygLeCMm2q9C0rguNMDCLOlFKSmpTVJiYmKtids5soJtFXcaAoOWoyFWYV4x5egTKBRzsA9LCwdLKxtjGwpI0TuTV0ahEaAAAACH5BAEAAD8ALAAAAAAQAAsAAAaFwN/vwfN4HsKk8ncA0TAa' +
												'QYNgWP5kIA6mM3sRCJfF4ZBMqWYlweKLqlU06YYLMaEMWKHVTVTRZkYwFBATEwgJFwwOPjEZGR0CPRMABQWGDBcAEAEkEREnGzo/DgAACQwMOZQTJgobEi1KBiE4Oxa2FhISNlZDAwUWHx8SALxKDggWCkg/QQA7' +
												'" width="16" height="11" />';
		
		cashMoscowIcon		=	'<img src="data:image/gif;base64' +
												',R0lGODlhEAAQANU6ACkFBt4aIvFCSdoaIaITGV8LDvNhZq8VG/mtsPbbAChtEekbI8AXHfFITvvIyu8oL/JRV7kWHNwaIfXkAca/E/AwOOcbI8fFJvA4P5iiGfJNU/A0O/NZX/aBhX0PE/V2e9W8Be8tNPFGTL0' +
												'WHcrDBPLPAPNbYeAaIt7bAoMPFIgQFeQbIveanUV5E/JYXmJbD7MVG+zMA22NC9XDD+8kLM0YH4EPE5IRFvA5QJ6rQQAAAAAAAAAAAAAAAAAAAAAAACH5BAEAADoALAAAAAAQABAAAAaMQJ1wSCwaj8hkERAZDAB' +
												'KT4Dl0GwIKZsxEvg4DKaGQjFZeQpDQmBDw+RaCtSloQmJbgUGg0DIgGQkc14ILhEETgMMFBMJGRIQCAgGEg8nBwsLNRQJEwcjARIYAQEdBzoABTAzJQkWFRUDAQ8dBlBDLyAxDwK8AgYcaEYAKgsCEBAcFkqnBwI' +
												'4tsvRSUEAOw==' +
												'" width="16" height="11" />';
		
		cashVegasIcon 		= '<img src="' +
												'data:image/png;base64,' +
												'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPA